...
class sw_2_linuxCNC_formatter():
    ...
    def _insert_safety_line(self):
        if(self._units and self._offset is not None):
            safty_line = ("{0} {1} {2} {3}".format(self.unit_dict[self._units],
                                                   self._offset,
                                                   self._work_plane,
                                                   self._spindle_mode))
            line_num = 0
            contents = ""
            codes = self.unit_dict.values()
            for code in codes:
                line = safty_line.replace(self.unit_dict[self._units], code)
                if line in self._file_contents:
                    self._file_contents = self._file_contents.replace(
                                               line, safty_line)
                    break
            if safty_line not in self._file_contents:
